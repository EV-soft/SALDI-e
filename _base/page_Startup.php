<?php   $DocFil='../_base/page_Startup.php';    $DocVer='5.0.0';    $DocRev='2018-01-00';     $DocIni='evs';  $ModulNr=0;
 ...
 */
  ...
  if ($noneconfig) {$pageTitl= 'Installation af SALDI programmet';}
...
### INDLÆS DATA:
    ...
    if (true) { // TEST:
      ...
      $db_encode= 'UTF8';
      $db_host= 'localhost';
      $db_bruger= 'root';  //$db_password;
      $db_password= 'geheimdb';
      //  $Ødb_Link= dbi_connect($sqhost, $squser, $sqpass, $Øsqdb);
      
    if ($Ødb_Type=="mysql") {
      echo '<br>Opretter database...';
      $Ødb_Link= dbi_connect($db_host, $db_bruger, $db_password, $db_navn,__FILE__, __LINE__);    
      //  var_dump($Ødb_Link);
      sql_creat("CREATE DATABASE ".$db_navn, __FILE__ . __LINE__);                                
      } else { /* PostgreS */                                                                     
        sql_creat("CREATE DATABASE ".$db_navn."WITH encoding = ".$db_encode, __FILE__ . __LINE__);
                                                                                                  
        dbi_DBclose($Ødb_Link, __FILE__ . __LINE__);                                              
        $Ødb_Link = dbi_connect($db_host, $db_bruger, $db_password, $Øsqdb,__FILE__, __LINE__);   
      }
    echo '<br>Opretter tabeller...';    make_Tables();
    echo '<br>Opretter indekser...';    make_Indexes();
    echo '<br>Database er klar...';
          SpalteTop(1100);  
