<?php      $DocFil= '../frasescann.php';    $DocVer='5.0.0';     $DocRev='2016-10-00';
...
/*  Denne fil skal køres manuelt fra SALDI-root mappen!  */
  $d = dir(".");  
  ...
  while (false !== ($entry = $d->read())) {
    ...
    if (is_dir($entry) ) {
      ...
      foreach ($files as $source) { $count= 0;  $search= "'@";  $search1= '"@';
        if ( ($source!=='.') and ($source!=='..') and (!strpos($source,'.bak')) and (!strpos($source,'scann.php')) and (!strpos($source,'.csv')) and (strpos($source,'.php')) ) {  
...
# foreach ($fraser as $frase) {if (strlen($frase[0])>3) echo '<br>'./* $x++.'  '. */trim($frase[0],'"');};
  ...
  echo '<br>Hvad nu ?<br>Kopier alle de linier der slutter med:    "","","","","","" til et tekstprogram der kan håndtere kolonne-kopiering og UTF-8 tegnsæt (f.eks. Notepad++)<br>';
  echo '<br>Kontroller at / juster alle "","","","","","" så de står nøjagtiget over hinanden. (Der må ikke være TAB-tegn eller andre spec.-tegn i filen!)<br>Kopier listen med de danske fraser uden @-prefix over i Google translate<br>';
  echo '<br>Start med at oversætte til Spansk. Indsæt resultatet i ""-kolonnen yderst til højre. <br>Derpå Tyrkisk i næstseneste kolonne, og Spansk rykker ud til højre.';
  echo '<br>Når alle kolonner er udfyldt, skal overflødige SPACE imellem " og " fjernes med søg/erstat, så kolonner overalt adskilles med "," <br><br>';
