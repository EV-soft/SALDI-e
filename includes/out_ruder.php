<?php			 $DocFil= '../includes/out_ruder.php';	 	$DocVer='5.0.0';		 $DocRev='2016-08-00';		 	$modulnr=0;
//						 ___   _   _    ___  _	       
//						/ __| /_\ | |  |   \| |   ___ 
//						\__ \/ _ \| |__| |) | |__/ -_)
...
#	if (file_exists($FaLogo)) echo '<img style="border:0px solid;width:50px;heigth:50px" alt="" src="'.$FaLogo.'">';
	...
	echo '<br>';
	if ($vis_finans) 	menuKnap($title='@Gå til menuen Kassekladder'				,$label='@Kasse kladder'	,$link='../_finans/kladdeliste.php'.$goBack);
	if ($vis_debitor) menuKnap($title='@Gå til menuen Debitor Ordrer'			,$label='@Ordrer'					,$link='../_debitor/ordreliste.php'.$goBack);
...
#	PROGRAM-MODUL;
function Rude_FootMenu() {
	echo '<div class="clearWrap"/>';	echo '<PanlFoot>';
		Foot_Links('<a style="color:green" href="'.$link='http://www.ev-soft.dk/saldi-wiki/doku.php?id=start'.'">'.
		'<u title="'.tolk('@Manual og anden hjælp finder du på SALDI-wiki').'">'.	tolk('@SALDI-wiki med manual').'</u></a>',
		$doPrint=true, $doErase=true, $doLookUp=true, $doAccept=true, $doExport=true, $doImport=true);
	echo '</PanlFoot>';
}
...
function Rude_DBsetup (&$db_type,&$db_encode,&$db_navn,&$db_bruger,&$db_password,&$adm_navn,&$adm_password,&$verify_adm_password) { 
	...
	htm_CombFelt($type='text',	$name='db_navn',			$valu= $db_navn,			$titl='@Ønsket navn på din hoveddatabase for SALDI. F.eks.: [saldi-db]',	$labl='@Databasenavn',	
							 $revi=true, $rows='2',$width='',$step='', $supp='required placeholder="'.tolk('@Udfyld...').'"');
	...
	htm_CombFelt($type='password',	$name='db_password',	$valu= $db_password,	$titl='@Adgangskode for ovenstående bruger',															$labl='@Adgangskode for databaseadministrator',	
							 $revi=true, $rows='2',$width='',$step='', $supp='required placeholder="'.tolk('@Password...').'"');
...
#	echo '<form>';
		...
		htm_CombFelt($type='password',	$name='confirm_password',	$valu= $verify_adm_password,	
								...
								$revi=true, $rows='2',$width='',$step='', $supp='required placeholder="'.tolk('@Gentag...').'"');
...
#	Test:
	if ($fp=fopen("../includes/connect.php","w")) {	fclose($fp); $inc='checked';}	else $inc.='';
...
#	echo '<div style="text-align: center"><img src= "../images/SALDIe50x150.png" alt="Saldi Logo" style="width:120px;heigth:80px;"></div><br>';
	...
	htm_FrstFelt('50%');	
	htm_CheckFlt($type='checkbox',$name='pg',	$valu= '',	$titl='@Systemet kontrollerer om modulet er tilgængeligt',	$labl='@Postgres findes.',	$revi=false,$supp=' '.$pg);
	htm_NextFelt('50%');	
	htm_CheckFlt($type='checkbox',$name='mysql',		$valu= '',	$titl='@Systemet kontrollerer om modulet er tilgængeligt',	$labl='@MySQL findes.',	$revi=false,$supp=' '.$mq);
#	htm_CheckFlt($type='checkbox',$name='mysql',		$valu= '',	$titl='@Systemet kontrollerer om modulet er tilgængeligt',	$labl='@MySQL findes.',	$revi=false,$supp='checked="'.var_dump(mysqli_init("")).'"');
	htm_LastFelt();
	echo '</div><hr>'.
	tolk('@Pakken med SALDI-filer, udpakkes i et arbejdskatalog med adgang for webbesøgende.').'<br><br>'.
	tolk('@Der skal være skriveadgang til 3 mapper:').'<br>';
	htm_FrstFelt('33%');
		htm_CheckFlt($type='checkbox',$name='incl',		$valu= '',	$titl='@Systemet kontrollerer om mappen er skrivbar',	$labl='/includes',	$revi=false,$supp=$inc);
	htm_NextFelt('33%');
		htm_CheckFlt($type='checkbox',$name='temp',		$valu= '',	$titl='@Systemet kontrollerer om mappen er skrivbar',	$labl='/temp',			$revi=false,$supp=$tmp);
	htm_NextFelt('33%');
		htm_CheckFlt($type='checkbox',$name='llib',		$valu= '',	$titl='@Systemet kontrollerer om mappen er skrivbar',	$labl='/logolib',		$revi=false,$supp=$lgo);
	htm_LastFelt();
	...
	htm_FrstFelt('50%');	
	htm_CheckFlt($type='checkbox',$name='hash',		$valu= '',	$titl='@Systemet kontrollerer om modulet er tilgængeligt',	$labl='@hash installeret.',	$revi=false,$supp='checked="'.extension_loaded('hash').'"');
	htm_NextFelt('50%');	
	htm_CheckFlt($type='checkbox',$name='mcrypt',	$valu= '',	$titl='@Systemet kontrollerer om modulet er tilgængeligt',	$labl='@mcrypt installeret.',	$revi=false,$supp='checked="'.extension_loaded('mcrypt').'"');
	htm_LastFelt();
	echo '<hr>'.
...
function Rude_Login (&$regnskab,&$brugernavn,&$brugerkode,&$ProgVers,&$LnkHelp,&$OrgaName,&$Logo) { 
	...
	htm_CombFelt($type='text',		$name='regn',	$valu= $regnskab,		$titl='@Angiv navnet på det Regnskab, som du har adgang til',	$labl='@Regnskab',		$revi=true, $rows='2',$width='',$step='', $supp='required="required" placeholder="'.tolk('@Regnskab...').'"');
	htm_CombFelt($type='text',		$name='navn',	$valu= $brugernavn,	$titl='@Angiv dit SALDI Brugernavn',													$labl='@Brugernavn',	$revi=true, $rows='2',$width='',$step='', $supp='required="required" placeholder="'.tolk('@Bruger...').'"');
	htm_CombFelt($type='password',$name='kode',	$valu= $brugerkode,	$titl='@Angiv Adgangskoden svarende til Brugernavnet',				$labl='@Adgangskode',	$revi=true, $rows='2',$width='',$step='', $supp='required="required" placeholder="'.tolk('@Password...').'"');
...
function Rude_Ydelser ($fakt) {
	htm_Rude_Top($name='yderform',$capt='@Ydelser / Produkter:',$parms='',$icon='fa-shopping-cart','panelWmax',__FUNCTION__);
...
function Rude_YdelserWide ($fakt) {
	echo '<div class="clearWrap"/>';	echo '<PanlFoot>';	
	htm_Rude_Top($name='linkform',$capt=tolk('@Ydelser / Produkter på salgsordren.').' ',$parms='',$icon='fa-shopping-cart','panelWmax',__FUNCTION__);
...
function Rude_Tabel() {
	...
	htm_Tabel($Titles= array(['@Ordre nr.','7%'],['@Ordre dato','10%'],['@Lev. dato','10%'],['@Konto nr.','15%'],['@Firma navn','30%'],['@Sælger','8%'],['@Ordre sum','30%']),
						$DataArr= array(
...
function Rude_Debitorer() {
	...
	htm_Tabel($Titles= array(
						['@Kontonr.','7%'],['@Firmanavn','10%'],['@Adresse','8%'],['@Sted','8%'],['@Postnr','4%'],['@By','8%'],['@Kontakt','12%'],['@Telefon','12%'],['@Sælger','12%']),
						$DataArr= array(
...
function Rude_Kreditorer() {
	...
	htm_Tabel($Titles= array(
						['@Leverandørnr.','7%'],['@Navn','10%'],['@Adresse','8%'],['@Sted','8%'],['@Post nr','4%'],['@By','8%'],['@Kontakt person','12%'],['@Telefon','12%']),
						$DataArr= array(
...
function Rude_KredOrdrer() {
	...
	htm_Tabel($Titles= array(['@Ordre nr.','6%'],['@Modt.nr.','6%'],['@Fakt.nr.','8%'],['@Ordre dato','6%'],['@Modt.dato','4%'],
						['@Konto nr.','8%'],['@Firma navn','12%'],['@Telefon','6%'],['@Leveres til','6%'],['@Vor ref.','8%'],['@Faktura sum','8%']),
						$DataArr= array(
...
function Rude_DbOrdrer() {
	...
	htm_Tabel($Titles= array(
						['@Ordre nr.','6%'],['@Ordre dato','6%'],['@Lev. dato','8%'],['@Konto nr.','6%'],['@Firma navn','24%'],['@Sælger','8%'],['@Ordre sum','6%']),
						$DataArr= array(
						['1025','Ordredato','Levdato','Kontonr.','Firmanavn','Sælger','Ordresum'],
						['1026','Ordredato','Levdato','Kontonr.','Firmanavn','Sælger','Ordresum'],
						['1027','Ordredato','Levdato','Kontonr.','Firmanavn','Sælger','Ordresum'],
...
#	PROGRAM-MODUL;
function Rude_TabelInp() {
	$dktip= tolk('@D/K  feltet benyttes i forbindelse med debitor- og kreditor posteringer. Er feltet tomt, betragtes det efterfølgende kontonummer som et Finans konto-nummer.').
	...
	$debkre= tolk('@Debet og Kredit-felterne er til kontonummeret på den konto, posteringen skal ske på.').
					tolk('@Afhængigt af koden i D/K vil der være tale om en debitor-, Kreditor- eller Finanskonto');
	htm_Rude_Top($name= 'kasseform',$capt= '@Kassekladde:',$parms='',$icon='','panelWmax',__FUNCTION__);
	htm_TabelInp(
		$Capt1= '@Kladde notat:',	
		$Capt2= '@Konto-kontrol:',
		$Titles= array(
			['@Bilag.', '5%',tolk('@Bilagsnummer tildeles automatisk og fortsættes fra sidst anvendte bilagsnummer fra samme bruger.').' '],
			['@Dato',  '10%',tolk('@Bilagets dato, som automatisk sættes til dags dato, men kan ændres.')],
			['@Bilags tekst','38%',tolk('@Bilagstekst er frivillig, men det er nyttigt senere at kunne se, hvad de enkelte posteringer drejer sig om.').' '],
			['@D/K',  '3.5%',$dktip],
			['@Debet', '10%',$debkre],
			['@D/K',  '3.5%',$dktip],
			['@Kredit','10%',$debkre],
			['@Faktura nr.','8%',tolk('@Fakturanr. benyttes i forbindelse med debitor- og kreditorposteringer.')],
			['@Beløb', '10%',tolk('@Beløb indeholder det beløb, der skal bogføres. Hvis man ved simulering eller anden kontrol opdager, ').
											tolk('@at en linje skal bogføres direkte modsat af, hvad der står i kassekladden,').' '.
											tolk('@så kan man blot sætte minustegn foran beløbet. På den måde bytter kontonumrene i felterne debet og kredit plads, og beløbet bliver igen positivt.')]),
			$RowPref= array(['<a href='.$link.' onclick=\"return confirm($confm)\"><img src=../icons/clip.png  alt="Clips" height="80%" width="80%" border=0></a>'],['Label'],['Tip'],['8%']),
...
function Rude_OrdrePostering() {
	...
	htm_TabelInp(
		...
		$Capt2= '@Saldo:',
		$Titles= array(
			['@Pos.',		 '5%',tolk('@Position tildeles automatisk.').' '],
			['@Varenr', '10%',tolk('@Varenummer hentes fra vareregistret.')],
			['@Antal',	 '5%',tolk('@Mængden af den aktuelle leverance.').' '],
			['@Enhed',   '5%',tolk('@Enhedsbeskrivelse af mængden')],
			['@Beskrivelse','40%',tolk('@Leverance beskrivlse')],
			['@Pris',		'10%',tolk('@Enhedspris')],
			['@Rabat%',	 '6%',tolk('@Rabatsats i %.')],
			['@Moms%',   '6%',tolk('@Moms %-sats for den posterede leverance')],
		#	['@Linie ialt','10%',tolk('@Beregnet beløb.')] tilføjes internt i htm_TabelInp
		),
...
function Rude_Formaal()	{
	...
	echo '<small><pre>';
	echo '  1. Forbedret modul-opbygget Code-design, så vedligeholdelse/udvikling bliver nemmere.<br>';
	...
	echo '     Mere her: [ http://php.net/manual/en/migration70.php ]<br>';
...
function Rude_Blindgyde() {
	...
	echo 'fordi det rigtige ikke er færdigudviklet.<br>';
	echo 'Gå tilbage ved at benytte browserens tilbageknap. <br><br>';
	echo '</small></div>';
	htm_RudeBund($pmpt='@Gem',$revi=false,$title='@Gem');
}
